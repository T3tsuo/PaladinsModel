...
async def main():
    async with arez.PaladinsAPI(DEV_ID, AUTH_KEY) as api:
        matches = api.get_matches_for_queue(arez.Queue.Casual_Siege, start=datetime.fromisoformat('2021-05-17'),
                                            end=datetime.fromisoformat('2021-05-18'), language=None, reverse=False,
        ...
        async for match in matches:
            # if the team won by a lot
            if abs(match.score[0] - match.score[1]) >= 3:
                try:
                    ...
                    for i in range(5):
                        ...
                        # checks if players have at least lvl 10 mastery champion
                        if champions_stat.level < 10:
                            print("skips")
                            notskip = False
                            break
                        ...
                        if champions_stat.level < 10:
                            ...
                            notskip = False
                            break
                    # if they're all lvl 10 and above then add data
                    if notskip is True:
                        print("adds")
                        sum1 = 0
                        sum2 = 0
